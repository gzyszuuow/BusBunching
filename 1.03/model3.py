...
for i in range(X1_train_stopfeatures.shape[0]):
    ls1 = []
    ls2 = []
    ls3 = []
    ls4 = []
    for index in range(0,28):
        ...
        l = list(X1_train_stopfeatures[i][index])

        ls1_each.append(l[0])
        ls2_each.append(l[1])
        ls3_each.append(l[3])

        for i in range(X1_train_stopfeatures.shape[2]):
            if i not in [0,1,3]:
                ls4_each.append(l[i])
        ls4.append(ls4_each)
...
X2_headways = np.array(X2_train_headway)
y_target_headway = np.array(y_train_target_headway)
RoutesNum = np.array(RoutesNum)
TimFeatures = np.array(TimFeatures)
StopFeatures = np.array(StopFeatures)
StopID = np.array(StopID)
